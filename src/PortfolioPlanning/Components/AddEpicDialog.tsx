...
> {
    ...
    private _onProjectChange = (index: number): void => {
        ...
        this.setState({selectedProject: {id: selectedItem.key.toString(), title: selectedItem.text}});
    ...
    private _renderEpicsPicker = () => {
        if (this.state.selectedProject) {
            ...
            return <div>The project {this.state.selectedProject.title} doesn't have any epic.</div>
        }
    };
    ...
    private _onEpicChange = (item: IDropdownOption): void => {
        ...
        if (item.selected) {
            ...
            const currIndex = newSelectedEpics.findIndex(epic => epic.id === item.key);
            if (currIndex > -1) {
